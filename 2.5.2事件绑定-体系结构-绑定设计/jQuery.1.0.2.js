...
(function(root) {
  ...
  Data.prototype = {
    getKey: function(elem) {
      ...
      if (!unlock) {
        ...
        Object.defineProperties(elem, descriptor); // elem: {唯一随机值： unlock}
        console.log('key elem[this.expando]:', elem[this.expando]);
    ...
    get: function(elem, property) {
      //找到或者创建缓存
      var cache = this.cache[this.getKey(elem)]; //1  {events:{},handle:function(){}} 
      //key 有值直接在缓存中取读
      return property === undefined ? cache : cache[property]; // cache: {events:{click:[type:'click',guid:n,handler:function]}}
    },
  }
  ...
  jQuery.event = {
    ...
    handlers: function(event, handlers) {
      // todo 这里应该可能添加了多个事件回调函数，不该只执行一个吧？
  ...
  jQuery.fn.extend({
    ...
    on: function(types, fn) {
      var type;
      if (typeof types === "object") { // 这里是支持什么场景？逻辑有问题吧？
        for (type in types) {
